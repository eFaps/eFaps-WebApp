define([
    ...
    "dojo/_base/sniff",
    "dojo/dom"
], function(domStyle, domClass, domGeometry, lang, _Module, declare, array, win, query, has, dom){
...
=====*/
    ...
    return declare(_Module, {
        ...
        _lockColumns: function(rowNode){
            this._lockColumnsInt(rowNode, rowNode.firstChild.rows[0]);
        },

        _lockColumnsInt: function(rowNode, _rows){
            // summary:
        ...
        _updateHeader: function(){
            ...
            if (rowNode.firstChild.rows.length > 1 && this.count > 0) {
                var countTmp = this.count;
                this.count = 1;
                this._lockColumnsInt(rowNode, rowNode.firstChild.rows[0]);
                this.count = countTmp;
                this._lockColumnsInt(rowNode, rowNode.firstChild.rows[1]);
                var gg = query('[groupid=' + this.grid.headerGroups[0].id + ']')[0];
                var groupWidth = 0;
                for(i = 0; i < this.grid._columns.length; i++){
                    if (this.grid._columns[i].groupId == this.grid.headerGroups[0].id) {
                        groupWidth = groupWidth + domGeometry.getMarginBox(dom.byId(this.grid._columns[i]._domId)).w;
                    }
                }
                groupWidth = groupWidth - domGeometry.getPadBorderExtents(gg).w;
                domStyle.set(gg, 'width', groupWidth + 'px');
            } else {
